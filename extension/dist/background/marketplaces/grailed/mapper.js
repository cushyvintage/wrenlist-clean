...
export class GrailedMapper {
    grailedClient;
    constructor(grailedClient) {
        this.grailedClient = grailedClient;
    }
    mapConditionToIndex(condition) {
        switch (condition) {
    ...
    async mapPhotos(productId, client) {
        const media = await getProductMediaForMarketplace(productId, "grailed");
        if (!media.length) {
            return [];
        }
        const uploadTasks = media.map((file) => async () => ({
            id: null,
            rotate: 0,
    ...
    buildShippingMatrix(product, addressCountryCode) {
        ...
        return {
            ...
            other: {
                amount: enabledDomesticCountries.includes(addressCountryCode)
                    ? worldwide
                    : domestic,
                enabled: hasWorldwide || !enabledDomesticCountries.includes(addressCountryCode),
            },
        };
    }
    async map(product) {
        const addresses = await this.grailedClient.getPostalAddressForAuthenticatedUser();
        if (!addresses || !addresses.data?.length) {
            throw {
                success: false,
                message: "Please add an address in your Grailed account under 'My Addresses' before posting.",
                type: "validation",
                internalErrors: "No address found in Grailed",
            };
        }
        const returnAddress = addresses.data.find((entry) => entry.default_return) ??
            addresses.data[0];
        const categorySlug = product.category[0]?.toLowerCase().trim() ?? "";
        const department = categorySlug.includes("women_")
            ? "womenswear"
            : "menswear";
        let designers = [FALLBACK_BRAND];
        if (product.brand) {
            const brand = (await this.grailedClient.getBrand(department, product.brand ?? "unbranded")) ?? FALLBACK_BRAND;
            designers = [brand];
        }
        const shipments = this.buildShippingMatrix(product, returnAddress.country_code);
        const hashtags = product.tags && product.tags.trim() !== ""
            ? product.tags
                .replaceAll("#", "")
                .split(",")
        ...
        const payload = {
            ...
            designers,
            exact_size: product.size && categorySlug.includes("womens_")
                ? product.size[0]
                : null,
            hashtags,
            ...
            shipping: shipments,
            size: product.size && !categorySlug.includes("womens_")
                ? product.size[0]
                : null,
            title: product.title,
            traits: [
        ...
        if (origin) {
            ...
            if (match?.value) {
                payload.traits.push({ name: "country_of_origin", value: match.value });
            }
        }
        return payload;
    }
