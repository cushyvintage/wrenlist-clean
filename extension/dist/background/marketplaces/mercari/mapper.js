...
export class MercariMapper {
    ...
    mapBrandId(name) {
        ...
        const brand = mercariBrands.find((entry) => entry.name.toLowerCase() === name.toLowerCase());
        return brand?.id ?? 0;
    }
    getZipCode(product) {
    ...
    ensurePriceCents(priceCents) {
        if (priceCents < MIN_PRICE_CENTS || priceCents > MAX_PRICE_CENTS) {
            throw new Error("Please enter the item price in the range supported by Mercari.");
        }
    }
    async buildPayload(product, options = {}) {
        ...
        const payload = {
            ...
            minPriceForAutoPriceDrop: product.smartPricing
                ? Math.ceil(Math.max(product.smartPricingPrice ?? product.price * 0.9, 1) * 100)
        ...
        const preferredCarrier = product.shipping?.preferredCarrier;
        if (product.shipping.shippingType === "Prepaid" &&
            ...
            const carrierResponse = await this.client.fetchCarriers(categoryId, shippingWeight, product.shipping.shippingHeight, product.shipping.shippingWidth, product.shipping.shippingLength);
            const available = carrierResponse?.availableShippingClassesV2?.shippingClasses ?? [];
            const filtered = available.filter((entry) => {
                const carrierMatch = typeof preferredCarrier === "string"
            ...
            if (!filtered.length) {
                throw new Error("No suitable carrier found for the given weight or preferred carrier.");
            }
            const selected = filtered.reduce((lowest, current) => !lowest || current.fee < lowest.fee ? current : lowest);
        ...
        if (payload.price - (payload.salesFee ?? 0) - sellerShippingFee <= 0) {
            throw new Error("After fees and shipping, this listing would result in a negative price.");
        }
