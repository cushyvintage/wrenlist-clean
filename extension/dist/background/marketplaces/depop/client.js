import { DEPOP_ADDRESSES_V1, DEPOP_ADDRESSES_V1_QUERY, DEPOP_DOMAIN, DEPOP_PICTURES_V2, DEPOP_POSTAL_ADDRESS, DEPOP_PRODUCT_VIEW, DEPOP_PRODUCTS_V1, DEPOP_PRODUCTS_V2, DEPOP_SHOP_PRODUCTS, } from "./constants.js";
...
export class DepopClient {
    ...
    async getPostalAddressForAuthenticatedUser(provider) {
        await this.ensureSession();
    ...
    async createPostalAddress(address) {
        await this.ensureSession();
    ...
    async postListing(payload) {
        await this.ensureSession();
        await this.storeCategories(payload.countryCode);
    ...
    async updateListing(marketplaceUrl, payload) {
        await this.ensureSession();
    ...
    async getListings(cursor, limit = 200) {
        await this.ensureSession();
        ...
        const products = response.products.map((product) => ({
            ...
            coverImage: product.pictures.length > 0
    ...
    async getListing(id) {
        await this.ensureSession();
        const response = await fetch(`${DEPOP_PRODUCT_VIEW}${id}/?lang=en`, {
            headers: {
                authorization: `Bearer ${this.bearerToken}`,
                "Content-Type": "application/json",
                accept: "application/json, text/plain, */*",
        ...
        const color = this.mapColor(product.colour?.[0]?.name ?? null);
        const color2 = this.mapColor(product.colour?.[1]?.name ?? null);
        ...
        return {
            ...
            images: product.pictures.length > 0
                ? product.pictures.slice(1).map((picture) => picture[picture.length - 1].url)
                : [],
            ...
            cover: product.pictures.length > 0
    ...
    generateTitle(product) {
        ...
        const gender = product.isKids ? "Kid's" : product.gender === "male" ? "Men's" : "Women's";
        const colorNames = product.colour
            ? product.colour.map((entry) => entry.name).join(" and ")
            : "";
        const productType = product.productType;
        return `${brand ? `${brand} ` : ""}${gender} ${colorNames ? `${colorNames} ` : ""}${productType}`;
    }
    mapConditionFromDepop(value) {
        switch (value) {
            case "brand_new":
    ...
    async storeCategories(countryCode) {
        try {
            await this.ensureSession();
