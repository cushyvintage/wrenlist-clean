...
const VEHICLE_ATTRIBUTE_MAP: Record<string, string> = {
...
export interface FacebookMapperDeps {
  uploadImage: (
    file: File,
    index: number,
  ) => Promise<{ payload?: { photoID: string } }>;
  fetchCarriers: (
    categoryId: string,
...
export interface FacebookCarrierOption {
...
export class FacebookMapper {
  private readonly currencyFallback = {
    US: "USD",
    UK: "GBP",
    CA: "CAD",
    AU: "AUD",
  ...
  constructor(private readonly deps: FacebookMapperDeps) {}

  private async mapPhotos(productId: string, isVehicle: boolean) {
    ...
    const uploadTasks = media.map(
      (file, index) => () => this.deps.uploadImage(file, index),
    );
  ...
  private mapColor(color?: string | null): SupportedColor | string | null {
    if (!color) {
      return null;
    }

    if (SUPPORTED_COLORS.includes(color as SupportedColor)) {
      return color;
    }
  ...
  private ensureLocation(shipping: ShippingInfo) {
  ...
  private buildDeliveryTypes(product: Product) {
    ...
    if ((product as Record<string, boolean>).publicMeetup) {
  ...
  private async applyPrepaidShipping(
    ...
    const preferredCarrier = (product as Record<string, unknown>)
      .preferredCarrier as string | undefined;
    const filtered =
    ...
    const selected = candidateList.reduce((lowest, current) =>
      !lowest || current.cost_amount < lowest.cost_amount ? current : lowest,
    );
  ...
  private applyOwnLabelShipping(
    ...
    const currency =
      this.currencyFallback[country as keyof typeof this.currencyFallback] ??
      "USD";
    ...
    payload.shipping_options_data = [
      {
        ...
        shipping_cost_option:
          shipping.sellerPays || shipping.domesticShipping === 0
            ? "SELLER_PAID_SHIPPING"
            : "BUYER_PAID_SHIPPING",
        shipping_cost_range_lower_cost: null,
        shipping_cost_range_upper_cost: null,
  ...
  public async map(product: Product) {
    ...
    const photos = await this.mapPhotos(
      product.id,
      product.category[0] === VEHICLE_CATEGORY_ID,
    );
    ...
    const payload: Record<string, any> = {
      ...
      product_hashtag_names:
        product.tags?.replaceAll("#", "").replaceAll(" ", "").split(",") ?? [],
      hidden_from_friends_visibility:
        (product as Record<string, boolean>).facebookShareWithFriends
          ? "VISIBLE_TO_EVERYONE"
          : "HIDDEN_FROM_FRIENDS",
