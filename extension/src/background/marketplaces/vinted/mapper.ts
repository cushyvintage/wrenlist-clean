...
const CONDITION_MAP: Record<Condition, number> = {
...
const VIDEO_RATING_MAP: Record<string, string> = {
  "AO - Adults only": "175",
  "E - Everyone": "170",
  "E10+ - Everyone 10+": "171",
  "M - Mature 17+": "173",
...
const CONSOLE_PLATFORM_MAP: Record<string, number> = {
  "Asus ROG Ally": 1259,
  Atari: 1260,
  Ayaneo: 1261,
  Commodore: 1262,
...
const RAM_MAP: Record<string, number> = {
  "2 GB": 3462,
  "4 GB": 3460,
  "8 GB": 3458,
  "12 GB": 3456,
...
const KEYBOARD_LAYOUT_MAP: Record<string, number> = {
  AZERTY: 1430,
  QWERTY: 1434,
  QWERTZ: 1435,
...
const SCREEN_SIZE_MAP: Record<string, number> = {
  '13.3"': 3497,
  '14"': 3496,
  '15.6"': 3496,
  '16"': 3495,
...
const OS_MAP: Record<string, number> = {
  macOS: 3487,
  Windows: 3488,
  "Chrome OS": 3489,
  Linux: 3490,
...
const STORAGE_CAPACITY_MAP: Record<string, number> = {
  "64 GB": 3480,
  "128 GB": 3478,
  "256 GB": 3474,
  "512 GB": 3470,
...
const INTERNAL_MEMORY_MAP: Record<string, number> = {
  "256 MB": 1292,
  "512 MB": 1293,
  "1 GB": 1294,
  "2 GB": 1295,
...
const SIM_LOCK_MAP: Record<string, number> = {
  "Network Locked": 1312,
  "Factory Locked": 1312,
  "Network Unlocked": 1313,
};

const CPU_MAP: Record<string, number> = {
  "Apple M1": 3393,
  "Apple M1 Pro": 3394,
  "Apple M1 Max": 3395,
  "Apple M2": 3396,
...
export class VintedMapper {
  ...
  constructor(
    private readonly vintedClient: VintedClient,
    private readonly tld: string,
  ) {}

  private async getPackageSize(
    shippingType: string | undefined,
    weight: { inGrams?: number; inOunces?: number } | undefined,
    catalogId: string,
  ): Promise<number> {
    ...
    if (this.tld === "co.uk") {
      ...
      const packageSizes = await this.vintedClient.fetchAvailablePackageSizes(
        catalogId,
      );
      ...
      const candidates: Array<{ limit: number; code: string }> = [
      ...
      for (const candidate of candidates) {
        if ((weight.inGrams ?? 0) <= candidate.limit) {
          const match = packageSizes.find(
            (pkg) => pkg.code === candidate.code,
          );
          if (match) {
            return match.id;
          }
    ...
    if (ounces <= 16) return 1;
    if (ounces <= 32) return 2;
    if (ounces <= 64) return 3;
  ...
  private async getPackageSizeForCategory(
    ...
    console.log('[Vinted Mapper] Available package sizes for catalog', catalogId, ':', 
    ...
    if (availableSizes.length === 0) {
      ...
      if (this.tld === "co.uk") {
        ...
        if (grams <= 500) return 1;   // Small
        if (grams <= 1000) return 2;  // Medium
        if (grams <= 2000) return 3;  // Large
        if (grams <= 5000) return 8;  // Heavy Small
        if (grams <= 10000) return 9; // Heavy Medium
    ...
    if (this.tld === "co.uk") {
      ...
      if (ounces <= 16) return availableSizes.find(p => p.id === 1)?.id ?? availableSizes[0].id;
      if (ounces <= 32) return availableSizes.find(p => p.id === 2)?.id ?? availableSizes[0].id;
      if (ounces <= 64) return availableSizes.find(p => p.id === 3)?.id ?? availableSizes[0].id;
  ...
  private isTitleValid(title: string): boolean {
    ...
    return !(
      /[A-Z]{7,}/.test(normalized) ||
      (len > 12 && uppercaseRatio >= 0.5)
    );
  }

  private async mapBrand(brand?: string | null) {
    if (!brand) {
    ...
    const match = results.find(
      (entry) => entry.title.toLowerCase() === brand.toLowerCase(),
    );
  ...
  private async mapColorToIndex(color?: string | null): Promise<number | null> {
    if (!color) return null;
    let alias = color;
    if (alias === "Gray") alias = "grey";
    if (alias === "Multi") alias = "various";
    if (alias === "Tan") alias = "khaki";
    if (alias === "LightBlue") alias = "light-blue";
    if (alias === "DarkGreen") alias = "dark-green";
    if (alias === "Beige") alias = "body";
    const match = COLOR_LOOKUP.find(
  ...
  private async mapPhotos(media: File[], tempUuid: string) {
    if (!media?.length) {
      return [];
    }

    const uploadTasks = media.map<() => Promise<VintedUploadedPhoto>>(
      (file) => () => this.vintedClient.uploadImage(file, tempUuid),
    );
    const results = await chunkConcurrentRequests(uploadTasks, 3);
    return results.map((photo) => ({ id: photo.id, orientation: 0 }));
  }

  private mapVideoRating(value: string | undefined) {
    return value ? VIDEO_RATING_MAP[value] : undefined;
  }

  private mapConsolePlatform(value: string | undefined) {
    return value ? CONSOLE_PLATFORM_MAP[value] : undefined;
  }

  private mapRam(value: string | undefined) {
    return value ? RAM_MAP[value] : undefined;
  }

  private mapKeyboardLayout(value: string | undefined) {
    return value ? KEYBOARD_LAYOUT_MAP[value] ?? KEYBOARD_LAYOUT_DEFAULT : undefined;
  }

  private mapScreenSize(value: string | undefined) {
    return value ? SCREEN_SIZE_MAP[value] : undefined;
  }

  private mapOs(value: string | undefined) {
    return value ? OS_MAP[value] : undefined;
  }

  private mapStorageCapacity(value: string | undefined) {
    return value ? STORAGE_CAPACITY_MAP[value] : undefined;
  }

  private mapInternalMemory(value: string | undefined) {
    return value ? INTERNAL_MEMORY_MAP[value] : undefined;
  }

  private mapSimLock(value: string | undefined) {
    return value ? SIM_LOCK_MAP[value] : undefined;
  }

  private mapCpu(value: string | undefined) {
  ...
  private async validateProduct(
    ...
    errors: string[];
    catalogId: number | null;
    brand: { id: number | null; title: string };
    media: File[];
  }> {
    const errors: string[] = [];
    ...
    let finalCatalogId: number | null = null;
    ...
    const hasExistingVintedPhotos = (product.vintedMetadata as any)?.photos?.length > 0;
  ...
  public async map(product: Product) {
    ...
    if (Array.isArray(precomputedColorIds) && precomputedColorIds.length > 0) {
      ...
      colorIds = precomputedColorIds.filter((id): id is number => typeof id === 'number' && id > 0);
    ...
    if (product.dynamicProperties?.ISBN) {
      ...
      try {
        const isbnAttributes = await this.vintedClient.fetchISBNInfo(
          product.dynamicProperties.ISBN,
        );
        const language = isbnAttributes.find(
          (attr) => attr.code === "language_book",
        );
        if (language) {
    ...
    if (product.dynamicProperties?.["Content rating"]) {
      payload.item.video_game_rating_id = this.mapVideoRating(
        product.dynamicProperties["Content rating"],
      );
    }

    if (product.dynamicProperties?.Platform) {
      const platformId = this.mapConsolePlatform(
        product.dynamicProperties.Platform,
      );
      if (platformId) {
        payload.item.item_attributes.push({
    ...
    if (product.dynamicProperties?.MaterialVinted) {
      const ids = (product.dynamicProperties.MaterialVinted as string).split("|").map(
    ...
    if (product.dynamicProperties?.["Operating system series"]) {
      const osId = this.mapOs(
        product.dynamicProperties["Operating system series"],
      );
      if (osId) {
        payload.item.item_attributes.push({
    ...
    if (product.dynamicProperties?.["Hard Drive Capacity"]) {
      const storageId = this.mapStorageCapacity(
        product.dynamicProperties["Hard Drive Capacity"],
      );
      if (storageId) {
        payload.item.item_attributes.push({
    ...
    if (product.dynamicProperties?.["Screen Size"]) {
      const screenId = this.mapScreenSize(
        product.dynamicProperties["Screen Size"],
      );
      if (screenId) {
        payload.item.item_attributes.push({
    ...
    if (product.dynamicProperties?.["Keyboard layout"]) {
      const keyboardId = this.mapKeyboardLayout(
        product.dynamicProperties["Keyboard layout"],
      );
      if (keyboardId) {
        payload.item.item_attributes.push({
    ...
    if (
      product.dynamicProperties?.["Storage Capacity"] &&
      (product.category[0] === "3661" || product.category[0] === "3728")
    ) {
      const internalId = this.mapInternalMemory(
        product.dynamicProperties["Storage Capacity"],
      );
      if (internalId) {
        payload.item.item_attributes.push({
  ...
  public normalizeColorValue(value?: string | null): string | null {
