...
interface VintedPackageSize {
  id: number;
  code: string;
}

interface VintedListingResponse {
  code?: number | string;
  message?: string;
  item?: { id: number };
  url?: string;
...
export class VintedClient {
  ...
  private createListingUrl: string;
  ...
  public getAnonId(): string { return this.anonId; }
  private catalogsCache: Array<{ id: number; title: string }> | null = null;
  ...
  public async bootstrap(force = false): Promise<void> {
    ...
    if (
      ...
      console.log("[Vinted] Using cached global tokens, age:",
        Math.round((Date.now() - VintedClient.globalTokens.lastRefresh) / 1000), "seconds");
      return;
    }
  ...
  private async detectAndUpdateTldFromCookies(): Promise<void> {
    try {
  ...
  private async updateHeaderRules(): Promise<void> {
    try {
      const navWithUA = navigator as Navigator & {
        userAgentData?: {
          getHighEntropyValues: (keys: string[]) => Promise<any>;
      ...
      const brands: Array<{ brand: string; version: string }> =
        uaData?.fullVersionList ?? navWithUA.userAgentData?.brands ?? [];
      ...
      try {
        ...
        const cookies: any[] = await new Promise((resolve) => {
          (chrome.cookies.getAll as any)({ domain, partitionKey: {} }, (result: any[]) => {
            resolve(result || []);
          });
        });
  ...
  private async setTokens(force = false): Promise<void> {
    try {
      ...
      if (
        ...
        this.username = VintedClient.globalTokens.username;
        this.uploadSessionId = VintedClient.globalTokens.uploadSessionId;
        console.log("[Vinted] Debug: Using global cache tokens, age:",
          Math.round((Date.now() - VintedClient.globalTokens.lastRefresh) / 1000), "s");
        return;
      }
      ...
      try {
        ...
        const allCookies = await chrome.cookies.getAll({});
        const vintedCookies = allCookies.filter(c => 
          c.domain.includes('vinted') && c.name === "v_uid"
        );
        ...
        if (vintedCookies.length > 0) {
          ...
          let allDomainCookies: chrome.cookies.Cookie[] = [];
          ...
          if (!this.username) {
            console.log("[Vinted] Debug: v_uid cookie not found. Available Vinted cookie names:", 
              allDomainCookies.filter(c => c.domain.includes('vinted')).map(c => `${c.name} (${c.domain})`).join(", "));
          }
        }
      } catch (cookieError) {
        ...
        if (stateMatches && stateMatches.length > 1) {
          try {
            ...
            if (userId) {
              ...
              match = ["", userId.toString()] as RegExpMatchArray;
            }
          } catch (e) {
      ...
      if (!this.username) {
        try {
          ...
          const wardrobeResponse = await fetch(
            `${this.apiUrl}/wardrobe/items?per_page=1`,
            {
              headers: {
  ...
  public async uploadImage(file: File, tempUuid: string) {
    await this.setTokens();
    ...
    return response as VintedPhotoResponse;
  }

  public async fetchBrands(keyword: string) {
    await this.setTokens();
    ...
    const brands: VintedBrand[] = response?.brands ?? [];
  ...
  public async fetchAvailablePackageSizes(catalogId: string): Promise<VintedPackageSize[]> {
    try {
      await this.setTokens();
      const response = await fetch(
        `${this.apiUrl}/catalogs/${catalogId}/package_sizes`,
        {
          headers: {
            Accept: "application/json, text/plain, */*",
            "X-Csrf-Token": this.csrfToken,
  ...
  public async fetchISBNInfo(isbn: string) {
    await this.setTokens();
    const response = await fetch(
      `${this.apiUrl}/item_upload/isbn_records?isbn=${isbn}`,
      {
        headers: {
          Accept: "application/json, text/plain, */*",
          "X-Csrf-Token": this.csrfToken,
    ...
    return response.attributes as Array<Record<string, any>>;
  }

  public async fetchConditions() {
  ...
  public async postListing(
    ...
    const response = await (async () => {
      ...
      try {
        return JSON.parse(text) as VintedListingResponse;
      } catch (err) {
    ...
    if (response.code === 131) {
      const address = shippingAddress as { zipCode: string } | undefined;
      if (!address?.zipCode) {
  ...
  public async updateListing(
    ...
    const res = await fetch(
      `${this.createListingUrl}/${marketplaceId}`,
      {
        method: "PUT",
        headers: {
          "Accept-Language": "en-us-fr",
    ...
    let response: VintedListingResponse;
    try {
      response = JSON.parse(text) as VintedListingResponse;
    } catch (e) {
  ...
  private handlePostErrors(
    response: VintedListingResponse,
  ): ListingActionResult | null {
  ...
  public async addShippingAddress(address: { zipCode: string }) {
    await this.setTokens();
   ...
   */
  public async getUserAddresses(retry = false): Promise<{ id: string; name?: string; isDefault?: boolean }[]> {
  ...
  public async storeCategories() {
    try {
      await checkAlreadyExecuted("categoryLastLoggedVinted", async () => {
        const info = (await getLoggingInfo("Category", "vinted", this.tld)) as {
          isLogged?: boolean;
        };
        ...
        if (catalogs?.catalogs?.some((entry: any) => entry.title === "Women")) {
          await log(
            "Category",
            JSON.stringify({
  ...
  public async fetchCatalogs(): Promise<Array<{ id: number; title: string }>> {
    if (this.catalogsCache) {
      return this.catalogsCache;
    }

    const fetchOnce = async () =>
      fetch(`${this.apiUrl}/catalogs`, {
        headers: {
          Accept: "application/json, text/plain, */*",
          "X-Csrf-Token": this.csrfToken,
          "X-Anon-Id": this.anonId,
  ...
  public async getCatalogId(categoryName: string): Promise<number | null> {
    try {
      ...
      // Try partial match
      const partialMatch = catalogs.find((c) =>
  ...
  public async getListings(
    ...
    _username?: string,
    retry = false,
    status: 'all' | 'active' | 'sold' | 'hidden' = 'all',
    ...
    const response = await fetch(
      `${this.apiUrl}/wardrobe/${this.username}/items?page=${pageNumber}&per_page=${perPage}&order=newest_first&currency=USD${condParam}`,
      {
        headers: {
          Accept: "application/json, text/plain, */*",
          "X-Csrf-Token": this.csrfToken,
    ...
    const products = body.items
      .filter((item: any) => !item.is_draft) // Only filter drafts, keep sold/closed items
      .map((item: any) => {
    ...
    return {
      products,
      nextPage:
        pageNumber === body.pagination.total_pages ? null : (pageNumber + 1).toString(),
      username: this.username,
      pagination: {
  ...
  public async getListing(id: string): Promise<Product | null> {
    ...
    const normalizeColor = (value?: string | null) => {
      ...
      if (value === "Dark green") return Color.DarkGreen;
      if (value === "Light blue") return Color.LightBlue;
    ...
    const photos = item.photos?.map((photo: any) => photo.url) ?? [];
    const shippingWeight = this.packageSizeIdToWeight(item.package_size_id);

    // Extract color IDs and titles
    const colorIds: number[] = [];
    const colorTitles: string[] = [];
    if (item.color1_id) colorIds.push(item.color1_id);
    if (item.color2_id) colorIds.push(item.color2_id);
    if (item.color1) colorTitles.push(item.color1);
    if (item.color2) colorTitles.push(item.color2);

    // Build comprehensive Vinted metadata for relisting
    const vintedMetadata: VintedImportMetadata = {
   ...
   */
  public async getConversationItems(
    conversationId: string,
    retry = false
  ): Promise<Array<{ itemId: string; title: string; price: number; thumbnailUrl?: string; itemUrl?: string }> | null> {
    await this.setTokens(retry);
  ...
  public async getConversationMessages(
    ...
    oppositeUser: { id: number; login: string } | null
    allowReply: boolean
  } | null> {
    await this.setTokens(retry);
    ...
    const messages = rawMessages.map((msg: any) => {
      ...
      if (entityType === 'message') {
        return {
          ...
          photos: entity.photos?.map((p: any) => p.url || p.thumbnails?.[0]?.url).filter(Boolean) || [],
  ...
  public async getSales(
    ...
    const parsePrice = (p: any): number => {
      ...
      if (typeof p === 'string') return safeParseFloat(p);
      if (p.amount) return safeParseFloat(p.amount);
      return 0;
    };
  ...
  public async getOrderDetails(
    ...
    let buyerAddressFromInvoicing: any = null;
    ...
    const safeParseFloat = (value: any): number => {
      ...
      if (typeof value === 'object' && value.amount) return parseFloat(value.amount) || 0;
      const parsed = parseFloat(value);
      return isNaN(parsed) ? 0 : parsed;
    };
   ...
   */
  public async getShipmentLabelOptions(
    shipmentId: string,
    retry = false
  ): Promise<ShipmentLabelOptions | null> {
    await this.setTokens(retry);
   ...
   */
  public async getNearbyDropOffPoints(
    shipmentId: string,
   ...
   */
  public async orderShipmentLabel(
    transactionId: string,
    labelType: string = 'printable',
    dropOffPointId?: string,
    sellerAddressId?: string,
    retry = false
  ): Promise<ShipmentLabelResult | null> {
    await this.setTokens(retry);
   ...
   */
  public async orderLabelAndPoll(
    transactionId: string,
    shipmentId: string,
    labelType: string = 'printable',
    dropOffPointId?: string,
   ...
   */
  public async getShipmentLabelDetails(
    shipmentId: string,
    retry = false,
    retryCount = 0
...
// Types for shipping label functionality
export interface ShipmentLabelOptions {
  ...
  // NOTE: Vinted API does NOT expose buyer shipping addresses for privacy.
  // Only name and photo are available. Full address is on the shipping label PDF.
  receiverInfo?: {
    name: string | null;
...
// Type for Vinted sales/transactions
export interface VintedSale {
  transactionId: string;
  conversationId?: string;
  status: string;
  statusCode?: number;
  statusTitle?: string;  // Human-readable status from Vinted
  ...
  // Shipping address (from invoicing info)
  shippingAddress?: {
    name?: string;
    line1?: string;
    line2?: string;
  ...
  trackingNumber?: string;
  carrier?: string;
  shipmentStatus?: number;        // Numeric status (e.g., 400 = delivered)
  shipmentStatusTitle?: string;   // Human-readable (e.g., "Package delivered.")
  shipmentStatusUpdatedAt?: string;  // When shipment status last changed (delivery date if delivered)
  labelUrl?: string;              // Shipping label PDF URL (if already generated)
