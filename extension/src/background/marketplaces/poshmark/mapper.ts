...
const TLD_SYMBOLS: Record<string, string> = {
  "co.uk": "£",
  ca: "C$",
  "com.au": "A$",
};
...
export interface PoshmarkBrandResult {
  brand_name?: string;
}

export interface PoshmarkMapperDeps {
  uploadImage: (file: File, order: number, listingId: string) => Promise<string>;
  getBrand: (
    department?: string,
    keyword?: string | null,
  ) => Promise<PoshmarkBrandResult[]>;
}

export interface PoshmarkPostPayload {
  id: string;
  title: string;
  description: string;
...
export class PoshmarkMapper {
  constructor(
    private readonly deps: PoshmarkMapperDeps,
    private readonly tld: string,
  ) {}

  private async brandExists(department?: string, brand?: string | null) {
    if (!department || !brand) {
      return false;
    }
    const results = await this.deps.getBrand(department, brand);
    return results.some(
      (entry) => entry.brand_name?.toLowerCase() === brand.toLowerCase(),
    );
  }

  private async mapImages(productId: string, listingId: string) {
    ...
    const uploadTasks = media.map(
      (file, index) => () => this.deps.uploadImage(file, index + 1, listingId),
    );
  ...
  public async map(
    ...
    const condition =
      product.condition === Condition.NewWithTags ? "nwt" : "not_nwt";
    ...
    const inventoryStatus =
      product.availability === "NotForSale" ? "not_for_sale" : "available";

    const payload: PoshmarkPostPayload = {
      id: listingId,
      title: product.title,
