...
// Etsy-specific result type for publishing operations
...
export class EtsyClient {
  ...
  public async getListings(
    ...
    const products: MarketplaceListingResult["products"] = listings.map((l) => ({
  ...
  public async crosslistProduct(
    ...
    try {
      ...
      const executionResult = result?.[0]?.result as { success: boolean; message?: string };
  ...
  try {
    ...
    if (!titleInput) {
      titleInput = document.querySelector(
        'input[name="title"], input[name="listing_title"], input[placeholder*="title"]'
      ) as HTMLInputElement | null;
    }
    ...
    let descriptionInput = document.querySelector(
      'textarea[aria-label*="description"], textarea[name="description"]'
    ...
    if (descriptionInput) {
      ...
      const elem = descriptionInput as HTMLElement;
      if ("value" in descriptionInput) {
        (descriptionInput as HTMLTextAreaElement).value = truncatedDesc;
    ...
    if (!priceInput) {
      priceInput = document.querySelector(
        'input[name="price"], input[type="text"][placeholder*="price"]'
      ) as HTMLInputElement | null;
    }
    ...
    if (tags) {
      const tagInputs = document.querySelectorAll(
        'input[aria-label*="tag"], input[name*="tag"]'
      );

      if (tagInputs.length > 0) {
        const tagList = tags.split(",").map(t => t.trim()).filter(Boolean);
        tagList.forEach((tag, index) => {
          const input = tagInputs[index] as HTMLInputElement;
          if (input) {
            input.value = tag;
