...
export default function RecentSearches({ searches, onReplay, isLoading }: RecentSearchesProps) {
  if (isLoading) {
    return (
      <Panel>
      ...
      <div className="space-y-2">
        {searches.map((entry) => (
          <button
            ...
            className="w-full text-left flex items-start gap-3 px-3 py-2 rounded hover:bg-cream-md transition group"
          >
            {entry.image_url ? (
              <img
                src={entry.image_url}
